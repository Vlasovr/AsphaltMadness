...
final class BackgroundView: UIView {
    ...
    lazy var getSingleLineWidth = {
        return self.asphaltView.frame.width / Constants.Game.bandsCount
    }
    ...
    private func setupShading() {
        ...
        let stripesLength = (totalRoadLength / (numberOfStripes * Constants.Game.oneAndHalf))
        
        listOfShadingViews.forEach { shadeView in
            
            for y in stride(from: stripesLength / Constants.Game.stripesSpacing,
                            to: self.frame.height - stripesLength / Constants.Game.stripesSpacing,
                            by: Constants.Game.oneAndHalf * stripesLength) {
