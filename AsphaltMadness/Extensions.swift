...
extension UIViewController {
    ...
    //MARK: - Starting and finishing game transition
    func fadeCurrentScreen(parrentView: UIView?, completion: @escaping () -> Void) {
        guard let parrentView else { return }
        
        let fadeView = UIView()
        parrentView.addSubview(fadeView)
        fadeView.frame = parrentView.frame
        fadeView.backgroundColor = .black
        fadeView.alpha = Constants.minimumAlphaValue
        parrentView.isUserInteractionEnabled = false
        
        UIView.animate(withDuration: Constants.Settings.fadeDurationOfAnimation) {
            fadeView.alpha = Constants.maxAlphaValue
        } completion: { _ in
            completion()
            parrentView.isUserInteractionEnabled = true
            DispatchQueue.main.asyncAfter(deadline: .now() + Constants.Settings.fadeDurationOfAnimation) {
    ...
    func launchScreenAnimation(parrentView: UIView?) {
        guard let parrentView else { return }
        
        let launchView = UIImageView()
        parrentView.isUserInteractionEnabled = false
        parrentView.addSubview(launchView)
        launchView.frame = parrentView.frame
        launchView.image = UIImage(named: Constants.launchScreenImage)
        
        UIView.animate(withDuration: Constants.Settings.launchSreenDurationOfAnimation) {
            launchView.alpha = .zero
        } completion: { _ in
    ...
    //MARK: - Stop and continue game animation
    ...
    func resumeLayer(layer: CALayer) {
        ...
        layer.speed = Constants.Settings.layerSpeed
